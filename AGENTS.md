...
- `link_section` comes from the nearest `data-track` container attribute (`header`, `footer`, `socials`, `share-links`, `related-posts`, `pagination`, `search-results`, `home-hero`, `courses`, `featured-posts`, `home-all-posts`, `posts-list`, `post-body`, `post-nav`, `post-tags`, `about`), falling back to `other`. When adding a new link-bearing section, put `data-track` on its container — never on individual anchors; `LinkButton.astro` does not forward extra props
